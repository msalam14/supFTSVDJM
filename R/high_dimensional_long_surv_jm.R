...
predict.supFTSVD_JM<-function(obj,designM,mc_ss=1000,surv_time,
  ...
  if(!is.null(new_dat)){
    ...
    if(constant_hazard){
      ...
      #bzH<-obj$CoxPH_obj$hazard[as.numeric(cut(surv_time,c(0,obj$CoxPH_obj$cum_hazard$time)))]
    }
...
#' 
#' @export
coxph_mle_fllik<-function(surv_time,censor_status,Xmat,max_iter=25,cnv_type="Param",cnv_crit=1e-6,constant_hazard=TRUE,change_points=NULL){
  n<-length(surv_time)
  p<-ncol(Xmat)
